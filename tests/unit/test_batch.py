...
def test_chunk_requests_splits_evenly():
    requests = [PromptRequest(custom_id=f"q{i}", prompt="test") for i in range(10)]
...
def test_chunk_requests_handles_remainder():
    requests = [PromptRequest(custom_id=f"q{i}", prompt="test") for i in range(12)]
    chunks = chunk_requests(requests, max_per_batch=5)
    assert len(chunks) == 3
    assert len(chunks[0]) == 5
    assert len(chunks[1]) == 5
...
def test_chunk_requests_single_chunk_if_under_limit():
    requests = [PromptRequest(custom_id=f"q{i}", prompt="test") for i in range(2)]
    chunks = chunk_requests(requests, max_per_batch=5)
    assert len(chunks) == 1
